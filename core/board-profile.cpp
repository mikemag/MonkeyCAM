    ...
    , m_debugPathSet("Profile")
{
  ...
  auto pointMarker = [&](DebugAnnotation& a, Point p, double len) {
    ...
    a.addSvgFormat(R"(<path d="M%f %f l%f 0"/>)",
                   p.X.dbl() + ls, p.Y.dbl(), len);
  ...
  m_debugPathSet.addAnnotation([&] {
      auto a = DebugAnnotation {
        DebugAnnotationDesc {
          ...
          "direction of the associated taper begin/end point.", "red", true
        }
      };
  ...
  m_debugPathSet.addAnnotation([&] {
      auto a = DebugAnnotation {
        DebugAnnotationDesc {
          "Nose, centers, tail", "", "blue", true
        }
      };
      auto guideWidth = (shape.minWidth() / 2 - 2).dbl();
      auto wMarker = [&](MCFixed x, double width) {
        a.addSvgFormat(R"(<path d="M%f %f L%f %f"/>)",
                       x.dbl(), -width, x.dbl(), width);
      };
